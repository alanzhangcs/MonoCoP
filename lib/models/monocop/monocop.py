...
def groupwise_random_route(chain, para, group_num, p_chain=0.5, mask=None, generator=None):
    """
    Group-wise random hard routing for training: selects either chain or para for each group as a whole.
    Input: chain/para [B, L, D] (L % group_num == 0)
    p_chain: probability of selecting chain
    mask: optional binary mask of shape [B, G, 1, 1]; if provided, reused without resampling
    Returns: (out, mask) where out has shape [B, L, D] and mask has shape [B, G, 1, 1]
    """
    assert chain.shape == para.shape and chain.ndim == 3
    B, L, D = chain.shape
    assert L % group_num == 0
    gL = L // group_num

    c = chain.view(B, group_num, gL, D)
    r = para.view(B, group_num, gL, D)

    if mask is None:
        probs = torch.rand((B, group_num, 1, 1), device=chain.device, generator=generator)
        mask = (probs < p_chain).to(chain.dtype)  # 1=选chain, 0=选para

    out = mask * c + (1.0 - mask) * r
    return out.reshape(B, L, D), mask


class MonoCoP(nn.Module):
    """ This is the MonoCoP module that performs monocualr 3D object detection """
    def __init__(self, backbone, depthaware_transformer, depth_predictor, num_classes, num_queries, num_feature_levels, 
                 aux_loss=True, with_box_refine=False, init_box=False, group_num=11, num_channels=[512, 1024, 2048], uncertainty_tau=0):
        ...
        self.depth_embed_para = MLP(hidden_dim, hidden_dim, 2, 2) # depth and deviation
        ...
        if with_box_refine:
            ...
            self.depth_embed_para = _get_clones(self.depth_embed_para, num_pred)
        else:
    ...
    def forward(self, images, calibs, targets, img_sizes, dn_args=None):
        ...
        for lvl in range(hs.shape[0]):
            ...
            # 1) 3D size
            size_feat = self.sizeNet[lvl](hs[lvl])
            ...
            # depth average + sigma
            if self.training:
                depth_reg, mask = groupwise_random_route(depth_reg_chain, depth_reg_para, self.group_num)
                depth_ave = torch.cat([depth_reg[:, :, 0: 1] + depth_geo.unsqueeze(-1) ,
                                      depth_reg[:, :, 1: 2]
                                    ], -1)
                outputs_depths.append(depth_ave)
            else:
                uncertainty_chain = depth_reg_chain[:, :, 1: 2]
                tau = getattr(self, "uncertainty_tau", 0)
                mask = uncertainty_chain < tau  # [B, Q, 1] bool
                depth = torch.where(mask, depth_reg_chain, depth_reg_para)  # [B, Q, 2]
                depth_ave = torch.cat([depth[:, :, 0: 1] + depth_geo.unsqueeze(-1) ,
                                        depth[:, :, 1: 2]], -1)
                outputs_depths.append(depth_ave)
...
def build(cfg):
    ...
    model = MonoCoP(
        ...
        uncertainty_tau=cfg.get("uncertainty_tau", 0))
