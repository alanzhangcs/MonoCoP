"""
    Official Waymo Open Dataset detection metric on KITTI-style outputs.

    This needs tensorflow + waymo_open_dataset, which do not coexist with the
    training environment, so it runs as its own process in a dedicated env.
    Waymo_Dataset.eval() shells out to it when `eval_type: "waymo"` is set;
    it can also be run by hand:

     <tf_env>/bin/python -u lib/datasets/waymo/waymo_eval.py \
         --root_dir data/WaymoDataset \
         --predictions outputs/<run>/outputs/data \
         --pd_set data/WaymoDataset/ImageSets/val_small.txt

     <tf_env>/bin/python -u lib/datasets/waymo/waymo_eval.py \
         --root_dir data/WaymoDataset --sanity   # APs should come out at 100.00

    `--pd_set` lists frame ids matching the detection filenames; each id is a
    line number into <root_dir>/ImageSets/val_org.txt, which maps it to the
    segment / frame the ground truth lives in.
"""
...
flags = tf.app.flags
FLAGS = flags.FLAGS
flags.DEFINE_string('root_dir',   'data/WaymoDataset', 'Dataset root holding ImageSets/ and validation_org/')
flags.DEFINE_string('predictions','','Directory of KITTI-style detection txt files')
flags.DEFINE_string('pd_set',     '','Split file listing the evaluated frame ids')
flags.DEFINE_bool('sanity', False, 'Feed the GT back in as predictions; APs should be 100.00')
...
if __name__ == '__main__':
    folder_path = _abs(FLAGS.root_dir)
